...
- Render zooms with a trackpad pinch (macOS), about the fingers' position, in the main window.
- Render's 3D rotation window is easier to navigate: the mouse wheel and trackpad (pinch included) zoom about the cursor, Shift + left drag zooms to a rectangle (drawn and cancelled like the main window's Zoom tool), the middle button or Alt (Option) + left drag pan like the right button, a double click fits the loaded region (Shift + double click also resets the rotation), Home and 1/2/3 fit and select the XY/XZ/YZ projections, and holding S snaps rotation to 15 degree steps.
