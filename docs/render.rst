...
- ``gpu: vram_budget_mb`` applies when rendering runs on the GPU (an experimental backend; how to switch it on is documented with that feature). Localizations are uploaded to the GPU once per dataset and kept there, so subsequent views render without any transfer; this key caps the memory (in MB) those uploads may occupy. When the cap is reached, the least recently rendered channels are released, and a single channel larger than the cap is rendered in pieces instead of failing. ``0`` removes the cap. As a rule of thumb, a two-dimensional dataset needs 16 bytes per localization (about 1 GB for 60 million localizations), a three-dimensional one with per-localization angles up to 28 bytes. On a GPU, the live previews during panning and zooming render the full data — ``interaction_subsample`` has no effect — because uploading a subset would cost more than rendering everything already resident.
