...
Everything renders on the GPU except *Global loc. prec.* combined with a 3D rotation, which stays on the CPU. The live previews during panning and zooming work exactly as on the CPU (``interaction_subsample``, counted over the localizations in the visible field of view) and are drawn straight from the localizations already resident on the GPU; the sharp image follows a moment after the gesture pauses. GPU and CPU images agree to well within display precision; the only difference is that the GPU sums the contributions to a pixel in a hardware-dependent order, so repeated renders can differ in the last decimal places of the raw intensities. Quantitative exports and the Python API (``picasso.render``) use whichever renderer the settings select, so set ``enabled: off`` if bit-for-bit reproducible CPU images are required.