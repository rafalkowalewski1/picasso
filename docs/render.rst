...
Everything renders on the GPU except *Global loc. prec.* combined with a 3D rotation, which stays on the CPU. The live previews during panning and zooming render the full data on the GPU — ``interaction_subsample`` has no effect — because uploading a subset would cost more than rendering everything already resident. GPU and CPU images agree to well within display precision; the only difference is that the GPU sums the contributions to a pixel in a hardware-dependent order, so repeated renders can differ in the last decimal places of the raw intensities. Quantitative exports and the Python API (``picasso.render``) use whichever renderer the settings select, so set ``enabled: off`` if bit-for-bit reproducible CPU images are required.